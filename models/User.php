...
class User
{
    ...
    public static function register($name, $email, $address, $password)
    {
        ...
        // Текст запроса к БД
        $sql = 'INSERT INTO user (name, email, address, password) '
     ...
     * @return boolean <p>Результат выполнения метода</p>
     */
    public static function edit($id, $name, $email, $address, $password)
    {
        // Соединение с БД
        $db = Db::getConnection();

        // Текст запроса к БД
        $sql = "UPDATE user SET name = :name,email = :email, address = :address, password = :password WHERE id = :id";
