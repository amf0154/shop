...
class Order
{
     ...
     */
    public static function save($userName, $userPhone, $userComment, $userId, $products, $address)
    {
        // Соединение с БД
        $db = Db::getConnection();

        // Текст запроса к БД
        $sql = 'INSERT INTO product_order (user_name, user_phone, user_comment, user_id, products, address) '
                . 'VALUES (:user_name, :user_phone, :user_comment, :user_id, :products, :address)';
    ...
    public static function getOrdersList()
    {
        ...
        $result = $db->query('SELECT id, user_name, user_phone, date, status, address FROM product_order ORDER BY id DESC');
        $ordersList = array();
        $i = 0;
        while ($row = $result->fetch()) {
            ...
            $ordersList[$i]['user_address'] = $row['address'];
            $i++;
        }
        return $ordersList;
    }
     ...
     * @param string $userAddress <p>Адрес клиента</p>
     * @param string $userComment <p>Комментарий клиента</p>
     * @param string $date <p>Дата оформления</p>
     * @param integer $status <p>Статус <i>(включено "1", выключено "0")</i></p>
     * @return boolean <p>Результат выполнения метода</p>
     */
    public static function updateOrderById($id, $userName, $userPhone, $userComment, $date, $status, $userAddress)
    {
        // Соединение с БД
        $db = Db::getConnection();
        ...
        $result->bindParam(':address', $userAddress, PDO::PARAM_INT);
        return $result->execute();
    }
