...
$to = "gleb@sgsrv.ru"; //получатель уведомлений
// не трогать
...
if ($interval < 1) { // интервал отправки (сек)
	...
	if (count($get_data) > 1) { // data to send
		...
		$title 	= "Заказ обратного звонка [shop.sgsrv.ru]";
		$title 	= "=?UTF-8?B?".base64_encode($title)."?=";
		$mess 	= "";

		$mess .= getOptions(1);
		$mess = $mess."<div style=\"background:#bfd4ac;border:1px solid #999;padding:10px;margin: 10px 0;\">IP: ".$ip." /</div>";

		$mess = $mess."<a href='http://shop.sgsrv.ru/'>Письмо было отправлено с shop.sgsrv.ru</a>";
		
		$headers	 = "Content-type: text/html; charset=utf-8\r\n"; 
		$headers	.= "From: shop.sgsrv.ru <".$from.">\r\n"; 
